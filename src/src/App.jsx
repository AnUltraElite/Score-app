...
import {
  ...
  checkInningsComplete, chaseStats, summarizeInnings, maxWicketsFor,
...
function CricketScorer({ m, color, visibility, onUpdate, theme, isDesktop, isOwner, onToast }) {
  ...
  const selectOutcome = (kind, extra) => {
    ...
    if (!m.striker || !m.nonStriker) return; // both batters required before any ball
  ...
  const startNextInnings = () => {
    onUpdate(m.id, (draft) => {
      const firstInnings = summarizeInnings(draft.a, draft.a.name);
  ...
  const scoringLocked = m.inningsComplete || needsOpeners;
  ...
  const scoringPad = (
    <>
      ...
      {isOwner && m.inningsComplete && popupDismissed && (
      ...
      <div>
        <Glass theme={theme} style={{ padding: "16px 16px 14px", position: "relative", overflow: "hidden", marginBottom: 10 }}>
          ...
          {innings === 2 && m.firstInningsSummary && (
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 10, fontSize: 11, color: theme.textFaint, position: "relative" }}>
